...
mod tests {
    ...
    fn test_statsd_client_gauge_with_timestamp() {
        ...
        assert_eq!("prefix.some.gauge:4|g|t:1234567890", res.unwrap().as_metric_str());
    }

    #[test]
    fn test_statsd_client_gauge_with_sampling_rate() {
        let client = StatsdClient::from_sink("prefix", NopMetricSink);
        let res = client.gauge_with_tags("some.gauge", 4).with_sampling_rate(0.5).try_send();

        assert_eq!("prefix.some.gauge:4|g|@0.5", res.unwrap().as_metric_str());
    }
